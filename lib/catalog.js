...
function parseOfficial(html,includeOutOfGame=false){
  ...
  const rows=String(html||'').match(/<tr\b[^>]*class=["'][^"']*player-row[^"']*["'][\s\S]*?<\/tr>/gi)||[];
  for(const row of rows){
    // Some active players can temporarily carry an out-of-game marker on the
    // quotations page. The normal catalogue keeps excluding those rows, while
    // search/sync can validate them against the current-season profile.
...
async function searchPlayers(q){
  ...
  if(official.length<100)throw new Error('Official Fantacalcio list unavailable');
  const legacyById=new Map(list.map(p=>[p.id,p]));
  const activeIds=new Set(official.map(p=>p.id));
  let merged=official.map(p=>{
  ...
  const excluded=await currentExcludedCandidates(q,activeIds);
  const byId=new Map();
  for(const p of [...merged,...excluded])if(p&&p.id&&!byId.has(p.id))byId.set(p.id,p);
  return [...byId.values()].filter(p=>p.name&&p.club&&p.role).sort((a,b)=>score(b,q)-score(a,q)).slice(0,8);
}
