...
const seeds=[{id:'5694',name:'Beto',club:'Fiorentina',role:'ATT',image:'https://content.fantacalcio.it/web/campioncini/21/medium/5694.png?v=640',profileUrl:'https://www.fantacalcio.it/serie-a/squadre/fiorentina/beto/5694'}];
...
async function loadList(){
  ...
  for(const u of LIST_URLS){
    ...
    if(Array.isArray(j)&&j.length>300){
      listCache=j.map(p=>({id:String(p.id),name:String(p.name||''),club:String(p.team||''),role:roleMap[p.position]||'',image:p.playerImage||'',profileUrl:''})).filter(p=>p.name&&p.club&&p.role);
...
async function enrich(p,existingRole=''){
  ...
  if(!h)return{...p,role:p.role||existingRole,profileUrl:url};
  ...
  return{...p,name:name||p.name,role,profileUrl:url};
}
async function searchPlayers(q){
  ...
  return merged.filter(p=>p.name&&p.club&&p.role);
}
