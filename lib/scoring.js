...
function build(p,directAll,newsAll,ctx,history){
  ...
  // A single scraped source must never collapse a player to 5%. Hard out requires
  // confirmation from at least two independent sources. This protects against
  // ambiguous "Out" labels and stale snippets while still reacting quickly to
  // confirmed injuries, suspensions and non-call-ups.
  const explicitSources=new Set([...directExplicit,...newsExplicit].map(x=>String(x.source||'').toLowerCase()).filter(Boolean));
  const hard=explicitSources.size>=2;
  ...
  else if(hard)summary=`Indisponibilità esplicita confermata da ${explicitSources.size} fonti indipendenti. Le indicazioni editoriali in conflitto sono escluse. Intervallo ${range.low}–${range.high}%.`;
